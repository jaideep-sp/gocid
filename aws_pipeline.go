...
// AwsConfig holds all the parameters needed for setup
...
func loadEnvConfig(envFile string) (AwsConfig, error) {
...
func createAwsClient(config AwsConfig) (aws.Config, error) {
...
func createSecurityGroup(ctx context.Context, client *ec2.Client, config AwsConfig) (string, error) {
...
func createIAMRole(ctx context.Context, config AwsConfig) (string, error) {
...
func launchEC2Instance(ctx context.Context, config AwsConfig) (string, string, error) {
	cfg, err := createAwsClient(config)
	if err != nil {
		return "", "", err
	}
...
// encryptSecret encrypts a secret value using the GitHub repository's public key
func encryptSecret(publicKey string, secretValue string) (string, error) {
	...
	encryptedBytes, err := box.SealAnonymous(nil, []byte(secretValue), (*[32]byte)(decodedKey), rand.Reader)
	if err != nil {
...
func setupGithubSecrets(ctx context.Context, config AwsConfig, publicIP, privateKeyPath string) error {
	...
	publicKey, _, err := client.Actions.GetRepoPublicKey(ctx, config.GithubOwner, config.GithubRepo)
	if err != nil {
		return fmt.Errorf("failed to fetch repository public key: %v", err)
	}
	...
	for name, value := range secrets {
		...
		_, err = client.Actions.CreateOrUpdateRepoSecret(
			ctx,
			config.GithubOwner,
			config.GithubRepo,
